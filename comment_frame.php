...
<body>
	...
	* {
	...
	if (isset($_POST['postComment' . $post_id])) {
		...
		// $insert_post = mysqli_query($con, "INSERT INTO comments VALUES ('', '$post_body', '$user_logged_in', '$posted_to', '$date_time_now', 'no', '$post_id')");
		$id = ''; // Auto-increments.
		$account_closed = 'no';
		$stmt = $mysqli->prepare("INSERT INTO comments VALUES (?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param("sssssss", $id, $post_body, $user_logged_in, $posted_to, $date_time_now, $account_closed, $post_id);
		$stmt->execute();
		$stmt->close();
		...
		while ($row = mysqli_fetch_array($get_commenters)) {
			...
			if ($row['posted_by'] != $posted_to && $row['posted_by'] != $user_to && $row['posted_by'] != $user_logged_in && !in_array($row['posted_by'], $notified_users)) {
				$notification = new Notification($con, $user_logged_in);
	...
	if ($count != 0) {
		...
		while ($comment = mysqli_fetch_array($get_comments)) {
			...
			// Timeframe.
			$date_time_now = date("Y-m-d H:i:s");
			$start_date = new DateTime($date_added); // Time of posting.
		...
		echo '<center style="background-color: #fff;"><br><br>No Comments to Show!</center>';
	}
